...
(function(window, undef) {
    ...
    var gid = 1,
        ...
        api = {
            ...
            uid: function(obj) {
                return obj ? (obj[api.expando] || api.uid()) : (++gid, api.expando + gid);
            },
                ...
                _each(items || [], function(item) {
                    ...
                    try {
                        if (entrySupport && _isEntry(item)) {
                            _readEntryAsFile(item, function(err, entryFiles, allEntries) {
                                if (err) {
                                    api.log('[err] getDropFiles:', err);
                                } else {
    ...
    function _readAs(file, fn, method, encoding) {
        if (api.isBlob(file) && _hasSupportReadAs(method)) {
            ...
            _emit(file, fn, 'error', undef, {
                error: 'filereader_not_support_' + method
            });
        }
    }
    ...
    api.addInfoReader(/^image/, function (file/**File*/, callback/**Function*/) {
        if (!file.__dimensions) {
            ...
            api.reasAsImage(file, function(evt) {
                var img = evt.target;
    ...
    api.event.dnd = function(el, onHover, onDrop) {
        ...
        if (FileReader) {
            // Hover
            _on(el, 'dragenter dragleave dragover', onHover.ff = onFover.ff || function(evt) {
